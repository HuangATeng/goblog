...
// New 分页对象构造器
// r -- 用来获取分页 URL 参数 默认是 page 可通过config/pagination.go 修改
// db -- GORM 查询句柄，用以查询数据集和获取数据总条数
// baseURL -- 分页链接
// PerPage -- 每页条数 传参为小于或等于 0 时为默认值 10 ，可通过 config/pagination.go修改
...
// Results 返回请求数据，注意： data 参数必须为GORM 模型Slice 对象
...
// PervPage 前一页码 0 意味着是第一页
func (p Pagination) PervPage() int  {
	hasPrev := p.HasPrev()
...
// GetPageFromRequest 从URL 中获取 page 参数
